...
@section('title',$deck->name . " - Fehér kártyák")

@section('content')
    <h2 class="page-header with-description">{{ $deck->name }} - Fehér kártyák</h2>
    ...
    <div class="row">
        <div class="col-md-4">
            <div class="card" style="margin-bottom:15px;">
                ...
                <div class="card-body">
                    <ul class="nav flex-column">
                        ...
                        <li class="nav-item">
                            <a class="nav-link active" href="{{ route('user.decks.deck.white', ['deck' => $deck]) }}">Fehér kártyák</a>
                        </li>
                        <li class="nav-item">
            ...
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title card-top">Fehér kártyák</h5>
                </div>
                <div class="card-body">
                    <ul class="nav flex-column">
                        <li class="nav-item">
                            <a class="nav-link active" href="{{ route('user.decks.deck.white', ['deck' => $deck]) }}">Összes kártya</a>
                        </li>
                        <li class="nav-item">
        ...
        <div class="col-md-8">
            <div class="row justify-content-center" style="margin-bottom:15px;">
                ...
                ?>
                @if($page>1)
                    <a href="{{ route('user.decks.deck.white', ['deck' => $deck, 'page' => 1]) }}" class="btn btn-sm btn-primary">1</a>&nbsp;
                    <a href="{{ route('user.decks.deck.white', ['deck' => $deck, 'page' => $page-1]) }}" class="btn btn-sm btn-primary">&laquo;</a>&nbsp;
                @else
                ...
                @if($page<$pages)
                    <a href="{{ route('user.decks.deck.white', ['deck' => $deck, 'page' => $page + 1]) }}" class="btn btn-sm btn-primary">&raquo;</a>&nbsp;
                    <a href="{{ route('user.decks.deck.white', ['deck' => $deck, 'page' => $pages]) }}" class="btn btn-sm btn-primary">{{ $pages }}</a>
                @else
            ...
            <div class="row">
                @foreach($cards as $card)
                    <div class="col-md-4">
                        <div class="card">
                            <div class="card-body" style="background-color:rgba(255,255,255,0.7); border-bottom-left-radius: 0; border-bottom-right-radius: 0;">
                                <h4 class="card-title" style="color:black;">{{ $card->getTextWhite() }}</h4>
                            </div>
                            <div class="card-footer" style="background-color:rgba(255,255,255,0.7)">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Új <i>fehér</i> kártya hozzáadása</h4>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
                <form action="{{ route('user.decks.deck.addWhite', ['deck' => $deck]) }}" method="POST">
                    {{ csrf_field() }}
            ...
            <div class="modal fade" id="edit-{{ $card->id }}">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form action="{{ route('user.decks.deck.addWhite', ['deck' => $deck, 'card' => $card]) }}" method="POST">
                            {{ csrf_field() }}
                            <div class="modal-body">
                                <div class="form-group" style="margin-bottom:-5px;">
                                    <div class="input-group">
                                    <span class="input-group-prepend">
                                        ...
                                        <input type="text" name="text" id="text" placeholder="Szöveg" class="form-control" required value="{{ $card->getTextWhite() }}">
                                    </div>
                                </div>
                            </div>
                            <div class="modal-footer">
